...
class MlflowInterface():
    ...
    def __init__(self):
        ...
        (self.mlflow_tracking_uri,
         self.mlflow_registry_uri) = self.get_mlflow_uris()
    ...
    def _search_experiment_by_name(self, name):
        try:
            ...
            raise HTTPException(502,
                                "Mlflow server error: cannot connect to "
                                "mlflow server")
        if experiment is None:
            raise HTTPException(404,
                                f"Experiment by name {name} does not exist")
        return experiment
    ...
    def get_model_version_info(self, name, version):
        ...
        if len(all_runs) < version:
            raise HTTPException(404,
                                "Version {} of experiment {} does not exist"
                                .format(version, name))
        elif version <= 0:
            raise HTTPException(400, "Model version cannot be less than 1")
        else:
    ...
    def _mlflow_inference(self, model_run, df):
        ...
        if df is None:
            raise HTTPException(404, "No data available for inference")
        try:
            loaded_model = mlflow.pyfunc.load_model(
                model_run.info.artifact_uri + "/" +
                json.loads(model_run.data.tags["mlflow.log-model.history"])[0][
                    "artifact_path"])
        except EndpointConnectionError:
            raise HTTPException(502,
                                "Minio Server Error: Cannot access "
    ...
    def _get_data_from_postgres(self, metadata):
        try:
            postgres = PostgresPandasWrapper(dbname=metadata.dbname,
                                             **self.postgres_data)
            postgres.connect()
            module = importlib.import_module(
                f"model_class.{metadata.filename}")
            ...
            if not isinstance(self.data_class_instance, ModelClass):
                raise HTTPException(
                    400,
                    "Requested class is not an instance of ModelClass"
                )
            queries = self.data_class_instance.get_query_for_prediction(
                metadata.user_id,
                metadata.project_id,
                metadata.starttime,
                metadata.endtime
            ...
            raise HTTPException(400, f"{e}")
        return self.data_class_instance.preprocess_data(
            raw_data, is_inference=True)
    ...
    def _insert_inference_data_in_postgres(self, metadata, inference_data):
        ...
        if not isinstance(data_class_instance, ModelClass):
            raise HTTPException(
                ...
                "Requested class is not an instance of ModelClass")
        try:
            postgres.insert_data(inference_data,
                                 data_class_instance.inference_table_name)
        except DataError:
            raise HTTPException(
                500,
                "Cannot upload infernece result to postgres db"
                )

    def _refine_return_obj(self, return_obj):
        # string to dictonary invocation_result column series
        return_obj["invocation_result"] = return_obj[
            "invocation_result"].apply(lambda x: json.loads(x))
        # Removing output_vector from invocation_result column dictionary
        # if output_vector is available
        return_obj["invocation_result"].apply(
            lambda x: x if "output_vector" not in x else x.pop(
                "output_vector"))
        return return_obj
    ...
    def get_inference_with_metadata(self, name, version, metadata, upload):
        ...
        if "alias" in experiment_run.data.tags:
            ...
            alias = "null"
        return_obj = self.data_class_instance.create_return_obj(
            df[1], name, version, alias, inference)
        if upload:
    ...
    def _get_best_model(self, name, metric):
        experiment = self._search_experiment_by_name(name)
        metrics_list = list(self.client.search_runs(
            experiment.experiment_id)[0].data.metrics)
        if metric is None:
            ...
            if metric not in metrics_list:
                raise HTTPException(
                    404,
                    f"{metric} metric is not a part of the model")
        best_model = self.client.search_runs(
            experiment.experiment_id,
            order_by=[f"metrics.{metrics_list[0]} ASC"])[0]
        version = self._search_model_version(best_model.info.run_id,
                                             experiment.experiment_id)
    ...
    def get_inference_from_best_model_with_metadata(self,
                                                    name,
                                                    metadata,
                                                    metric,
                                                    upload):
        ...
        if "alias" in experiment_run.data.tags:
            ...
            alias = "null"
        return_obj = self.data_class_instance.create_return_obj(df[1],
                                                                name,
                                                                version,
                                                                alias,
                                                                inference)
    ...
    def _get_latest_model(self, name):
        experiment = self._search_experiment_by_name(name)
        all_experiments = self._get_all_experiment_runs(
            experiment.experiment_id)
    ...
    def get_inference_from_latest_model_with_metadata(self,
                                                      name,
                                                      metadata,
                                                      upload):
