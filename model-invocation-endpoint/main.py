...
def get_inference_from_best_model(name: str,
                                  data: DataInputModel, metric: str = None):
...
def get_inference_from_latest_model_with_metadata(
                                            name: str,
                                            metadata: DataLoaderClass,
                                            upload: Optional[bool] = True
                                            ):
    return mlflow_interface.get_inference_from_latest_model_with_metadata(
            name, metadata, upload)


@app.post("/models/{name}/best/metadata-invocation/")
def get_inference_from_best_model_with_metadata(name: str,
                                                metadata: DataLoaderClass,
                                                metric: Optional[str] = None,
                                                upload: Optional[bool] = True):
    return mlflow_interface.get_inference_from_best_model_with_metadata(
                                                            name,
                                                            metadata,
                                                            metric,
                                                            upload
...
def get_inference_with_metadata(name: str,
                                version: int,
                                metadata: DataLoaderClass,
                                upload: Optional[bool] = True):
    return mlflow_interface.get_inference_with_metadata(name,
                                                        version,
                                                        metadata,
                                                        upload)
